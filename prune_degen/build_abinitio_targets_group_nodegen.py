...
# Need to group the molecules together, or else we won't have a reference conformer.
# Then loop over grouped molecules

# loop over molecules, create a reference molecule if the molecule hasn't been seen (by inchi?)
# then remap, and remap gradient
...
def create_targets(records_and_molecules, target_path):
    ...
    #
        ...
        try:
            ...
            # Here is where degeneracies are filtered out
            dEs = energies - min(energies)
            nondegen_idx = np.logical_not(np.isclose(dEs,0,atol = 1e-6))
            nondegen_idx[np.argmin(energies)] = True

            if np.count_nonzero(nondegen_idx) < nondegen_idx.shape[0]:
                print('Energies: ',energies)
                print('dEs:', dEs)
                print('non degenerate:',nondegen_idx)
                print(gradients.shape)
                print(xyzs.shape)
                energies = energies[nondegen_idx]
                gradients = gradients[nondegen_idx]
                xyzs = xyzs[nondegen_idx]
                record_ids = record_ids[nondegen_idx]
                print(energies)
                print(gradients.shape)
                print(xyzs.shape)
                molecule_from_record_remap.to_file('degenerate_mols/Abinitio_QCA-' + str(
            i) + '-' + molecule.hill_formula+'.sdf','SDF')


            if len(energies) == 1:
                print("Single molecule after removing degeneracies, need at least two conformers for relative energy, ignoring this record", key)
                targets_excluded.write(f"QCA Record ID:{record.id}, SMILES: {molecule.to_smiles(mapped=True)}\n")
                continue


            # Check for molecules that would be set to 0 due to being too high energy in the attenuation step
            # Don't need to filter out because FB does that--but remove molecules that would become single conformers due to this.
            upper = 0.012748809  # 33.472 kJ/mol to Ha -- in FB, if dE is higher than this, weight is set to 0
            attenuate_keep_idx = dEs < upper

            if np.count_nonzero(attenuate_keep_idx) == 1: # if only one conformer is below the full attenuation cutoff
                print("Single conformer after attenuation, need at least two conformers for relative energy, ignoring this record", key)
                targets_excluded.write(f"QCA Record ID:{record.id}, SMILES: {molecule.to_smiles(mapped=True)}\n")
                continue
